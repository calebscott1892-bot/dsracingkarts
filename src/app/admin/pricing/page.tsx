...
export default function AdminPricingPage() {
  ...
  async function handlePreview() {
    ...
    if (!pct || pct <= 0) { setErrorMsg("Enter a valid percentage greater than 0."); return; }
    setLoading(true); setErrorMsg(""); setSuccessMsg(""); setPreview(null);
    try {
  ...
  async function handleApply() {
    const pct = parseFloat(percentage);
    if (!pct || pct <= 0) return;
    if (!confirm(`Apply a ${pct}% price increase to ALL products in Square? This cannot be undone.`)) return;
    setApplying(true); setErrorMsg(""); setSuccessMsg("");
    try {
  ...
  const totalVariations = preview?.reduce((sum, p) => sum + p.variations.length, 0) ?? 0;
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-text-muted text-sm mb-8">
        Increase all product prices in Square by a percentage. Prices update in Square first, then sync to the website automatically.
        Prices are rounded to the nearest $0.05.
      </p>
      ...
      <div className="card p-6 mb-6">
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-1 min-w-[160px] max-w-[240px]">
            ...
            <div className="relative">
              <input
                ...
                step="0.1"
                value={percentage}
                onChange={(e) => { setPercentage(e.target.value); setPreview(null); setSuccessMsg(""); setErrorMsg(""); }}
                placeholder="e.g. 5"
          ...
          {preview && (
            ...
            >
              {applying ? <Loader2 size={16} className="animate-spin" /> : <TrendingUp size={16} />}
              {applying ? "Applying…" : `Apply to Square`}
            </button>
          )}
        </div>

        {successMsg && (
          <div className="mt-4 flex items-start gap-2 text-green-400 text-sm">
      ...
      {preview && (
        <div>
          <p className="text-text-muted text-sm mb-3">
            Showing new prices for <strong className="text-white">{totalVariations} variation{totalVariations !== 1 ? "s" : ""}</strong> across{" "}
            <strong className="text-white">{preview.length} product{preview.length !== 1 ? "s" : ""}</strong> — a{" "}
            <strong className="text-brand-red">{percentage}% increase</strong>. Click "Apply to Square" above to confirm.
          </p>
                ...
                {preview.map((product) => (
                  <>
                    {product.variations.length > 1 && (
                      <tr key={`header-${product.id}`} className="bg-surface-700/30 border-t border-surface-700">
                        <td colSpan={4} className="px-4 py-2 font-medium text-white text-xs uppercase tracking-wider">
                          {product.name}
                        </td>
                      </tr>
                    )}
                    {product.variations.map((v) => (
                      <tr key={v.id} className="border-t border-surface-700/50 hover:bg-surface-700/20">
                        <td className="px-4 py-3 text-white">
                          {product.variations.length === 1 ? product.name : (
                            <span className="pl-2 text-text-muted">{v.name || "Default"}</span>
                          )}
                        </td>
